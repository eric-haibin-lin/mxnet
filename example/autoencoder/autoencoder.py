...
class AutoEncoderModel(model.MXModel):
    ...
    def layerwise_pretrain(self, X, batch_size, n_iter, optimizer, l_rate, decay, lr_scheduler=None, monitor_count=100):
        ...
        solver.set_monitor(Monitor(monitor_count))
    ...
    def finetune(self, X, batch_size, n_iter, optimizer, l_rate, decay, lr_scheduler=None, monitor_count=100):
        def l2_norm(label, pred):
           return np.mean(np.square(label-pred))/2.0
        solver = Solver(optimizer, momentum=0.9, wd=decay, learning_rate=l_rate, lr_scheduler=lr_scheduler)
        solver.set_metric(mx.metric.CustomMetric(l2_norm))

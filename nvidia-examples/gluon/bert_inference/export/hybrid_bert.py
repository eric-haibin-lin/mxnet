...
__all__ = ['HybridBERTModel', 'HybridBERTEncoder',
           ...
           'init_fast_multiheadattn_and_softmax']
...
# Do not use masked softmax workaround from gluonnlp
def masked_softmax(F, att_score, mask, dtype):
...
def fast_init(self, base_cell, query_units, key_units, value_units, num_heads, use_bias=True,
              weight_initializer=None, bias_initializer='zeros', prefix=None, params=None):
    super(nlp.model.attention_cell.MultiHeadAttentionCell, self).__init__(prefix=prefix, params=params)
    self._proj_in_units = query_units
    self._num_heads = num_heads
    self._use_bias = use_bias
    assert key_units == self._proj_in_units
    assert value_units == self._proj_in_units
    assert self._proj_in_units % self._num_heads == 0
    with self.name_scope():
        self.dropout_layer = nn.Dropout(base_cell._dropout)
        self.proj_in = nn.Dense(units=self._proj_in_units * 3, use_bias=self._use_bias,
                                flatten=False, weight_initializer=weight_initializer,
                                prefix='proj_in')

def fast_forward(self, F, query, key, value, mask=None):
    ...
    att_weights = self.dropout_layer(masked_softmax(F, att_score, mask, self._dtype))
    output = F.interleaved_matmul_selfatt_valatt(qkv_proj, att_weights, heads=self._num_heads)
    return output, att_weights

def init_fast_multiheadattn_and_softmax():
    nlp.model.attention_cell._masked_softmax = masked_softmax
    nlp.model.attention_cell.MultiHeadAttentionCell.__init__ = fast_init
    nlp.model.attention_cell.MultiHeadAttentionCell.hybrid_forward = fast_forward
...
class HybridBaseTransformerEncoder(HybridBlock):
    ...
    def __init__(self, attention_cell='multi_head', num_layers=2,
                 ...
                 seq_length=None, prefix=None, params=None, dtype='float16'):
    ...
    def hybrid_forward(self, F, inputs, states=None,
        ...
        if valid_length is not None:
            mask = F.cast(valid_length, dtype='int32')
...
def get_hybrid_bert_model(model_name=None, dataset_name=None, vocab=None,
                          ...
                          seq_length=None, use_FP16=True, **kwargs):
...
class HybridBERTForQA(HybridBlock):
    ...
    def __init__(self, bert, prefix=None, params=None, use_FP16=False):
    ...
    def hybrid_forward(self, F, inputs, token_types, valid_length=None):
        ...
        """
        valid_length = F.cast(valid_length, self.dtype)
        bert_output = self.bert(inputs, token_types, valid_length)
        output = self.span_classifier(bert_output)
        out1, out2 = F.split(output, axis=2, num_outputs=2)
        return out1, out2

...
def check_elementwise_sum_with_shape(shape, n):
    # forward
    inputs = [mx.symbol.var('arg%d' % i) for i in range(n)]
    out = mx.symbol.ElementWiseSum(*inputs, name='esum')
    arr = [mx.nd.empty(shape) for i in range(n)]
    arr_grad = [mx.nd.empty(shape) for i in range(n)]
    for i in range(n):
        arr[i][:] = np.random.uniform(-10, 10, shape)
    exec1 = bind(out, default_context(),
                 args=arr,
                 args_grad=arr_grad)
...
def test_python_op():
    ...
    exec1 = bind(s, default_context(), args=[x], args_grad = {'X': dx})
    exec1.forward(is_train=True)
...
def test_swapaxes():
    ...
    exe_c = bind(swap, default_context(), args=[arr_data])
    ...
    for shape, axis1, axis2 in config:
        ...
        exe_c = bind(ret_mx, default_context(), args=[data_mx])
        exe_c.forward(is_train=True)
        out = exe_c.outputs[0]
...
def test_shape_array():
    for i in range(1,6):
        ...
        ya = np.shape(xa)
        yg = mx.nd.ones(ya)
        exe = bind(y, ctx=default_context(), args={'x': xa},
                   args_grad={'x': xg})
...
def test_binary_op_duplicate_input():
    ...
    out_grad[:] = 1
    square = data * data
    exe_square = bind(square, default_context(), args=[arr_data], args_grad=[arr_grad])
...
def test_sign():
    ...
    exe_test = bind(test, default_context(), args=[arr_data], args_grad=[arr_grad])
    exe_test.forward(is_train=True)
...
def test_round_ceil_floor():
    ...
    exe_test = bind(test, default_context(), args=[arr_data])
    exe_test.forward(is_train=True)
    out = exe_test.outputs[0]
...
def test_maximum_minimum():
    ...
    exe_test = bind(test, default_context(), args=[arr_data1,arr_data2], args_grad=[arr_grad1,arr_grad2])
    exe_test.forward(is_train=True)
    out = exe_test.outputs[0]
...
def test_maximum_minimum_scalar():
    ...
    exe_test = bind(test, default_context(), args=[arr_data1], args_grad=[arr_grad1])
    exe_test.forward(is_train=True)
    out = exe_test.outputs[0]
...
def check_deconvolution_forward_backward(input_shape, num_filter, kernel, stride, pad):
    ...
    exe = bind(deconv, default_context(), args=args, args_grad=args_grad)
    exe.forward(is_train=True)
    out = exe.outputs[0]
    ...
    exe = bind(deconv, default_context(), args=args, args_grad=args_grad_addto, grad_req="add")
    exe.forward(is_train=True)
...
def check_deconvolution_gradient(input_shape, num_filter, pad):
    ...
    exe_conv = bind(conv, default_context(), args=conv_args, args_grad=conv_args_grad)
    ...
    exe_deconv = bind(deconv, default_context(), args=deconv_args, args_grad=deconv_args_grad)
    exe_deconv.forward(is_train=True)
    deconv_out_grad = conv_data[:]
    exe_deconv.backward(deconv_out_grad)
    assert_almost_equal(conv_args_grad[1], deconv_args_grad[1], rtol=1e-3, atol=1e-2)
    # Test AddTo
    exe_deconv_addto = bind(deconv, default_context(), args=deconv_args,
...
def check_nearest_upsampling_with_shape(shapes, scale, root_scale):
    ...
    exe = bind(up, default_context(), args=arr, args_grad=arr_grad)
    exe.forward(is_train=True)
...
def test_convolution_independent_gradients():
    ...
    for dim, num_base, kernel_x, stride_x, pad_x , in_size, no_bias in \
        ...
        for req_kind in reqs:
            ...
            exe1 = bind(conv, ctx, args1, args_grad=grad1, grad_req=grad_req1)
            exe1.forward(is_train=True)
            exe1.backward(exe1.outputs[0])
            ...
            for x_req, w_req, b_req in itertools.product(reqs, repeat=3):
                ...
                exe2 = bind(conv, ctx, args2, args_grad=grad2, grad_req=grad_req2)
...
def check_binary_op_forward(symbol, baseline, gen_data, rtol=1e-3, atol=1e-5, mx_nd_func=None):
    ...
    for i in range(sample_num):
        d = gen_data(i)
        y = bind(symbol, default_context(), args={'a': mx.nd.array(d[0]), 'b': mx.nd.array(d[1])})
...
def check_binary_op_backward(symbol, baseline, gen_data, rtol=1e-3, atol=1e-5):
    ...
    for i in range(sample_num):
        ...
        y = bind(symbol, default_context(), args={'a': mx.nd.array(d[0]), 'b': mx.nd.array(d[1])},
                 args_grad=[y_1, y_2])
        y.forward(is_train=True)
        y.backward([mx.nd.array(out, dtype=out.dtype)])
...
def test_run_convolution_dilated_impulse_response(dil=(1,1), kernel_shape=(3,3), verbose=False):
    ...
    net.list_arguments()
    be = bind(net, default_context(), args={ 'input' : spike_img, 'test_convolution_weight' : kernel_weights},
    ...
    be = bind(net, default_context(), args={ 'input' : input_grad, 'test_convolution_weight' : kernel_weights})
    be.forward(True)
    out_o = be.outputs[0].asnumpy()
    ...
    be = bind(net, default_context(), args={ 'input' : white_in, 'test_convolution_weight' : rnd_kernel},
    ...
    be = bind(net, default_context(), args={ 'input' : white_in, 'test_convolution_weight' : dkernel})
...
def test_reduce():
    ...
    def test_reduce_inner(numpy_reduce_func, numpy_reduce_grad_func, mx_reduce_sym, nan_prob=0,
        ...
        for i in range(sample_num):
            ...
            net = bind(b, default_context(), args={'a': mx.nd.array(dat_npy)},
                       args_grad={'a': grad_nd})
...
def test_broadcast():
    ...
    for i in range(sample_num):
        ...
        def test_broadcasting_ele(sym_bcast):
            ...
            net = bind(sym_bcast, default_context(), args={'a': mx.nd.array(dat_npy)},
                       args_grad={'a': grad_nd})
            net.forward(is_train=True)
...
def test_slice_axis():
    for ndim in range(1, 6):
        ...
        for t in range(ndim):
            ...
            exec1 = bind(Y, default_context(), args = [x], args_grad = {'X': xgrad})
            exec1.forward(is_train=True)
            y = exec1.outputs[0]
            assert_allclose(x.asnumpy()[idx], y.asnumpy())
            exec1.backward([y])
            ...
            exec2 = bind(Y, default_context(), args=[x], args_grad={'X': xgrad}, grad_req="add")
...
def test_slice_like():
    for ndim in range(1, 6):
        ...
        for t in range(ndim):
            ...
            exec1 = bind(Y, default_context(), args = [x, x1],
        ...
        for c in [1, 2, 3, 4]:
            for h in [5, 9, 13, 17]:  # for convenience test, this third and forth input dim should be 4x + 1
                for w in [5, 9, 13, 17]:
                    ...
                    exe = bind(stn, dev, args=args, args_grad=grad_grad)
                    exe.forward(is_train=True)
                    out = exe.outputs[0]
                    # check forward
...
def check_pad_with_shape(shape, xpu, pad_width, mode, dtype="float64"):
    ...
    exec1 = bind(Y, xpu, args = [x], args_grad = {'X': grad})
    exec1.forward(is_train=True)
    out = exec1.outputs[0]
    # compare numpy + mxnet
...
def check_instance_norm_with_shape(shape, xpu):
    ...
    exec1 = bind(Y, xpu, args = {'X':x, 'G':gamma, 'B':beta})
...
def check_sequence_reverse(xpu):
    ...
    def test_wrapper(arr, xpu, sequence_length=None, use_sequence_length=False):
        ...
        if sequence_length:
            bound = bind(rev, xpu, {'seq': mx.nd.array(arr), 'seq_len': mx.nd.array(sequence_length)})
        else:
            bound = bind(rev, xpu, {'seq': mx.nd.array(arr)})
        fwd = bound.forward()
...
def test_cast_float32_to_float16():
    ...
    def check_cast(op, input_np, expected_output):
        ...
        ctx = default_context()
        exe = bind(sym, ctx, {'x': mx.nd.array(input_np, dtype=np.float32, ctx=ctx)})
...
def test_amp_multicast():
    ...
    exe = bind(res, ctx, {'x': mx.nd.random.uniform(shape=(3, 3), dtype=np.float16, ctx=ctx),
    ...
    def check_amp_multicast(input_np, expected_output):
        ...
        ctx = default_context()
        res = mx.sym.amp_multicast(x, y, z, num_outputs=3)
        exe = bind(res, ctx, {'x': mx.nd.array(input_np, dtype=np.float16, ctx=ctx),
...
def test_all_finite():
    ...
    ctx = default_context()
    exe = z.bind(z, ctx, {'data': inf_arr})
    exe.forward(is_train=False)
    sym_output = exe.outputs[0].asnumpy()
    assert sym_output[0] == 0
    exe = z.bind(z, ctx, {'data': finite_arr})
    exe.forward(is_train=False)
    sym_output = exe.outputs[0].asnumpy()
    assert sym_output[0] == 1
    z = mx.sym.multi_all_finite(data, data2, num_arrays=2)
    exe = z.bind(z, ctx, {'data': finite_arr, 'data2': inf_arr})
    exe.forward(is_train=False)
    sym_output = exe.outputs[0].asnumpy()
    assert sym_output[0] == 0
    z = mx.sym.multi_all_finite(data, data2, num_arrays=2)
    exe = z.bind(z, ctx, {'data': finite_arr, 'data2': finite_arr})
    exe.forward(is_train=False)
    sym_output = exe.outputs[0].asnumpy()
    assert sym_output[0] == 1
...
def test_repeat():
    ...
    def test_repeat_backward(axis):
        ...
        exe = bind(test, ctx=default_context(), args=[arr_data], args_grad=[arr_grad])
...
def test_softmax_with_large_inputs():
    def softmax_forward(input_data, true_output):
        ...
        exec1 = bind(out1, default_context(), args={'data': input_data})
...
def check_ctc_loss(acts, labels, loss_truth, contrib=False):
    ...
    exe = bind(ctc, ctx=default_context(), args=[acts_nd, labels_nd])

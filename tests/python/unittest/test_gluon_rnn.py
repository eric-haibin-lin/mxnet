...
def test_lstm_cpu_inference():
    ...
    y = model(x)
    y_cell = model_cell.unroll(2, x, layout='TNC', merge_outputs=True)[0]

    assert_almost_equal(y_cell, EXPECTED_LSTM_OUTPUT, rtol=1e-3, atol=1e-5)
    assert_almost_equal(y, EXPECTED_LSTM_OUTPUT, rtol=1e-3, atol=1e-5)
...
def check_rnn_forward(layer, inputs, deterministic=True):
    ...
    if deterministic:
        assert_almost_equal(np_out, out, rtol=1e-3, atol=1e-5)
        assert_almost_equal(np_dx, input_grads, rtol=1e-3, atol=1e-5)
...
def check_rnn_layer_forward(layer, inputs, states=None, run_only=False, ctx=mx.cpu()):
    ...
    if not run_only:
        assert_almost_equal(np_out, out, rtol=1e-3, atol=1e-5)
        assert_almost_equal(np_dx, inputs.grad, rtol=1e-3, atol=1e-5)
...
def test_rnn_layers_fp32():
    ctx = mx.Context.default_ctx
    run_rnn_layers('float32', 'float32', ctx)

@assert_raises_cudnn_not_satisfied(min_version='5.1.10')
@unittest.skipIf(mx.context.num_gpus() == 0, "RNN FP16 only implemented on GPU for now.")
def test_rnn_layers_fp16():
    ctx = mx.Context.default_ctx
    # At this point, we know there are GPUs in the system. However, the test will be invoked
    # directly with a cpu context and via import into ./tests/python/gpu/test_gluon_gpu.py
    # with a gpu context.  We prune out the first case here:
    if ctx == mx.cpu():
        print('Bypassing RNN FP16 test on the cpu- only implemented on GPU for now.')
    else:
        run_rnn_layers('float16', 'float32', ctx)

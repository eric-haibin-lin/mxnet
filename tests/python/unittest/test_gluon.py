...
def test_parameter_dict():
    ...
    assert_almost_equal(prev_w0, cur_w0)
    assert_almost_equal(prev_w1, cur_w1)
...
def test_parameter_row_sparse_data():
    ...
    assert_almost_equal(retained_0, retained_target_0)
    ...
    assert_almost_equal(retained_1, retained_target_1)
    ...
    assert_almost_equal(retained_2[0], retained_target_2)
...
def check_layer_forward(layer, dshape):
    ...
    assert_almost_equal(np_out, out, rtol=1e-5, atol=1e-6)
    assert_almost_equal(np_dx, x.grad, rtol=1e-5, atol=1e-6)
...
def test_sync_batchnorm():
    ...
    for ndev, cuda in cfgs:
        ...
        for shape in [(N, 2), (N, 3, 4), (N, 4, 4, 4), (N, 5, 6, 4, 4)]:
...
def check_split_data(x, num_slice, batch_axis, **kwargs):
    ...
    assert_almost_equal(mx.nd.concat(*res, dim=batch_axis), x)
...
def test_req():
    ...
    with mx.autograd.record():
        ...
        l = loss(pred, label)
        l.backward()
        grad = net[0][0].weight.grad().mean()
        # run twice to check req = add
        pred = net(data)
        l = loss(pred, label)
        l.backward()

    grad_double = net[0][0].weight.grad().mean()
...
@with_seed()
def test_zero_grad():
    def test_func(ctx, dtype='float32', sparse=False, embeddingType=None):
        ...
        for type in ['float16', 'float32', 'float64']:
            for embType in ['float32', 'float64']:
                for sparse in [True, False]:
                    test_func(ctx, dtype=type, sparse=sparse, embeddingType=embType)
...
def check_layer_forward_withinput(net, x):
    ...
    with mx.autograd.record():
        out2 = net(x_hybrid)
    out2.backward()
    assert_almost_equal(x.grad, x_hybrid.grad, rtol=1e-5, atol=1e-6)

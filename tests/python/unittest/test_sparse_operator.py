...
def test_elemwise_binary_ops():
    ...
    with warnings.catch_warnings():
        ...
        for ii in range(1):
            ...
            # Try different tensor shapes
            shape = rand_shape_2d()

            # This test can take a long time to run, depending on the tensor shape.  Currently, each
            # dimension is limited to a max of 10, but lets cut down on the total size (and hence
            # runtime) without reducing the max dimension of 10.
            max_size = 40
            while (np.prod(shape) > max_size):
                shape = rand_shape_2d()

...
from test_operator import discard_stdout, discard_stderr
...
def test_large_models():
    ...
    def tensor_size(big_tensor_bytes):
        bytes_per_float = 4
        cudnn_tensor_elements_max = 2 * 1024 * 1024 * 1024 - 1
        tensor_elements = min(big_tensor_bytes / bytes_per_float, cudnn_tensor_elements_max)
        sz = int(math.sqrt(tensor_elements / largest_num_features))
        return (sz // 100) * 100
...
def _test_gluon_bulking_in_process(seed, time_per_iteration, bound_core_id=None):
    ...
    num_ops = 2000
    core_msg = ''
    # pinning all threads to the same core enhances the consistency of the measurements.
    if bound_core_id is not None and sys.platform.startswith('linux'):
        num_iterations = 10
        # with discard_stdout():
        os.system('taskset -p -c {} {}'.format(bound_core_id, os.getpid()))
        core_msg = 'On core {:2d}, '.format(bound_core_id)
    ...
    iteration_time()
    times = np.array([iteration_time() for _ in range(num_iterations)])
    mean_time = times.mean()
    std_dev_time = times.std()
    best_time = times.min()
    format_str = \
        '    {}(best, mean, stddev) = ({:.4f}, {:.4f}, {:.8f}).  Trials = ' + len(times) * '{:4f} '
    print(format_str.format(core_msg, best_time, mean_time, std_dev_time, *times))
    sys.stdout.flush()
    time_per_iteration.value = best_time


@with_seed()
def _test_bulking(test_bulking_func, max_trials=1):
    sleep_secs = 60
    for trial in range(max_trials):
        try:
            # test case: (max_fwd_segment_size, max_bwd_segment_size, enable_bulking_in_training)
            blksz = 200
            test_cases = [(blksz,blksz,True),
                          (blksz,1,True), (1,blksz,True),
                          (0,0,True), (blksz,blksz,False)]
            times = {}
            times_str = ''
            print()
            for seg_sizes in test_cases:
                ...
                enable_core_bind = trial < max_trials - 1
                bound_core_id = np.random.randint(mp.cpu_count()) if enable_core_bind else None
                # The test may confine all threads to a core, which effects thread shutdown order
                # and creates occasional benign shutdown errors.  We quiet these here.
                # Also, saw an issue where pinning to a core makes the stream-create fail with
                ...
                if not run_in_spawned_process(test_bulking_func,
                                      {'MXNET_EXEC_BULK_EXEC_MAX_NODE_TRAIN_FWD' : seg_sizes[0],
                                       ...
                                       'MXNET_GPU_WORKER_NSTREAMS' : 1,
                                       'MXNET_GPU_WORKER_NTHREADS' : 1},
                                      time_per_iteration, bound_core_id):
                    # skip test since the python version can't run it properly.  Warning logged.
                    print('run_in_spawned_process failed.')
                    return
                times[seg_sizes] = time_per_iteration.value
                times_str += \
                    '\n    runtime of op seg bulking (fwd,bwd,enable)=({},{},{}):\t{:.1f} msec'.\
                        format(seg_sizes[0], seg_sizes[1], seg_sizes[2], 1000.0 * times[seg_sizes])
            times_str += '\n'
            fastest_non_bulked_time = min(times[(0,0,True)], times[(blksz,blksz,False)])
            slowest_half_bulked_time = max(times[(1,blksz,True)], times[(blksz,1,True)])
            fastest_half_bulked_time = min(times[(1,blksz,True)], times[(blksz,1,True)])
            fully_bulked_time = times[(blksz,blksz,True)]

            # Force output, so we can guage how close this test is to failing, even when it passes.
            sys.stderr.write(times_str)
            sys.stderr.flush()
            # Non-bulked times[0,0,True], times[1,1,True] and times[blksz,blksz,False] should be
            # slower than both half-bulked times[1,blksz,True] and times[blksz,1,True]
            ...
            # The fully bulked times[blksz,blksz,True] should be faster than both half-bulked runs
            ...
            # Test passed, so exit outer trial loop early
            return
        except AssertionError as e:
            if trial < max_trials - 1:
                print('\nSaw test failure: {}.'.format(e))
                # Test could have been effected by competing cpu/gpu activity.  A wait may help.
                print('Retrying after {} secs...'.format(sleep_secs))
                time.sleep(sleep_secs)
            else:
                # All trials failed, so signal failure with this last exception.
                raise e

@with_seed()
def test_bulking():
    max_trials = 4
    _test_bulking(_test_gluon_bulking_in_process, max_trials)

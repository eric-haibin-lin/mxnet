...
# The bulking test has been seen to fail with CUDNN_STATUS_INTERNAL_ERROR. 
# Trying the test first in case it interacts poorly with previously-run tests.

# isolated execution bulking test function to be invoked with different env var settings
def _test_bulking_in_process(seed, time_per_iteration, bound_core_id=None):
    num_iterations = 25
    data_shape = (16,)
    num_ops = 2000
    core_msg = ''
    ...
    # presence of interruptions like python garbage collection, we use the fastest time as a gauge.
    iteration_time()
...
def test_bulking():
    max_trials = 4
    _test_bulking(_test_bulking_in_process, max_trials)
...
def test_report_sm_archs():
    sys.stderr.write('GPU SM archs = {} ... '.format(
        [get_sm_arch(i) for i in range(mx.context.num_gpus())] ))
...
def check_fast_lars(w_dtype, g_dtype, shapes, ctx, tol1, tol2):
    ...
    npy_w_sum_sq = mx.nd.array([(w.astype('float32') ** 2).sum() for w in weights_arr],
                                dtype='float32', ctx=ctx)
    npy_g_sum_sq = mx.nd.array([(g.astype('float32') ** 2).sum() for g in grads_arr],
                                dtype='float32', ctx=ctx)

    assert_almost_equal(npy_w_sum_sq, w_sum_sq, atol=tol1, rtol=tol1)
    assert_almost_equal(npy_g_sum_sq, g_sum_sq, atol=tol1, rtol=tol1)

    rescale_grad = (np.random.rand() + 0.5) * 100.
    mx_new_lrs = mx.nd.multi_lars(mx_lrs, w_sum_sq, g_sum_sq, mx_wds, eta=eta, eps=eps,
                                  rescale_grad=rescale_grad)

    npy_w_l2norm = mx.nd.sqrt(npy_w_sum_sq)
    npy_g_l2norm = mx.nd.sqrt(npy_g_sum_sq * rescale_grad * rescale_grad)
    new_lrs = mx.nd.zeros(npy_w_l2norm.shape, dtype='float32', ctx=ctx)
    for i in range(npy_w_l2norm.size):
        ...
        if _w > 0.0 and _g > 0.0:
            ...
            new_lrs[i] = lrs[i]
    assert_almost_equal(new_lrs, mx_new_lrs, atol=tol2, rtol=tol2)
...
# Helper function for test_batchnorm_with_type and test_batchnorm_relu_fusion
def _batchnorm_versions_helper_fp16(batchnorm_op_list, data, fix_gamma, use_global_stats, use_relu):
    ctx_list = []
    sym_list = []
    (n, h, w, c) = data
    # Set up an atol of 0.2 for the largest test case (128, 56, 56, c). Otherwise scale.
    atol = 0.2 * n * h * w / (128 * 56 * 56)
    rtol = 1e-2
    atol_mult = atol / rtol
    # BatchNorm gpu cudnn (if cudnn is enabled)
    if 'batchnorm_cudnn' in batchnorm_op_list:
        ctx_list.append({'ctx': mx.gpu(0), 'batchnorm_data': data,
                         'type_dict': {'batchnorm_data': np.float16}, 'atol_mult': atol_mult})
        bn = mx.sym.BatchNorm(fix_gamma=fix_gamma,
                              use_global_stats=use_global_stats,
                              name='batchnorm', cudnn_off=False)
        if use_relu:
            bn = mx.sym.Activation(data=bn, act_type='relu', name='batchnorm')
        sym_list.append(bn)
    ...
    if 'batchnorm_cudnn_transposed' in batchnorm_op_list:
        ...
        act = 'relu' if use_relu else None
        batchnormed = mx.sym.BatchNorm(data=transposed, fix_gamma=fix_gamma,
                                       use_global_stats=use_global_stats,
                                       name='batchnorm', cudnn_off=False, axis=ndim-1, act_type=act)
    ...
    check_consistency(sym_list, ctx_list, use_uniform=True, scale=0.4, tol=rtol)
...
def test_batchnorm_with_type():
    ...
    exp_data_shape = (4, 8, 8, 16)
    ctx_list_v2_2D_exp = [
        {'ctx': mx.gpu(0), 'norm_data': exp_data_shape, 'type_dict': {'norm_data': np.float16}},
        {'ctx': mx.gpu(0), 'norm_data': exp_data_shape, 'type_dict': {'norm_data': np.float32}},
    ]
    ...
    check_consistency(sym, ctx_list_v2_2D_exp, use_uniform=True, scale=0.5)
    ...
    if cuda_arch < 60:
        ...
        for shape_nchw in nchw_shapes:
            ...
            for fix_gamma in [False, True]:
                ...
                _batchnorm_versions_helper_fp16(batchnorm_op_list=['batchnorm_cudnn',
...
def test_bilinear_sampler_versions():
    ...
    for item in test_cases:
        ...
        for exe in exe_list:
            ...
            exe.forward(is_train=True)
            assert_almost_equal(exe_list[ref_idx].outputs[0], exe.outputs[0], rtol=1e-3, atol=1e-5)

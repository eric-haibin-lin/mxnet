...
# TODO(cfujitsang): need more extensive grid test
def test_slice():
    data = mx.sym.Variable('data')
    data_shape = (1, 128, 1000)
    sym = mx.sym.slice(data, begin=(None, None, None), end=(None, None, -1))
    check_single_sym(sym, {'data': data_shape}, rtol_fp32=0., atol_fp32=0.,
                     rtol_fp16=1e-3, atol_fp16=1e-7)
    sym = mx.sym.slice(data, begin=(None, 64, None), end=(None, None, None))
    check_single_sym(sym, {'data': data_shape}, rtol_fp32=0., atol_fp32=0.,
                     rtol_fp16=1e-3, atol_fp16=1e-7)

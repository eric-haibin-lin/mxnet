...
class ThreadedEngine : public Engine {
 ...
 private:
  ...
  std::string to_string(const BulkStatus& bulk_status) const {
    if (bulk_status.count == 0) {
      return std::string("ImperativeBulk");
    }
    std::string repr("ImperativeBulk: ");
    repr += std::string(bulk_status.names[0]);
    for (int i = 1; i < bulk_status.count; ++i) {
      repr += " + " + std::string(bulk_status.names[i]);
    }
    return repr;
  }
  ...
  inline void BulkAppend(SyncFn exec_fn, Context exec_ctx,
    ...
    bulk_status.names.push_back(name);
  ...
  inline void BulkFlush() {
    ...
    // We want the bulk_name to be freed only after all the functions have been executed
    // Hence the lambda capture of a shared_ptr
    std::shared_ptr<std::string> bulk_name = std::make_shared<std::string>(
                                                              to_string(bulk_status));
    // Need this here because PushAsync calls BulkFlush
    bulk_status.count = 0;
    this->PushAsync([functions, bulk_name](RunContext ctx,
      ...
      FnProperty::kNormal, 0, bulk_name->c_str());
    bulk_status.functions.reset(new std::vector<SyncFn>());
    bulk_status.functions->reserve(bulk_status.bulk_size);
    bulk_status.names.clear();

...
inline Engine::OprHandle CreateEngineOp(
    ...
    const std::string& seg_names,
    std::unordered_set<std::string> *opr_segs_names) {
  ...
  auto it = opr_segs_names->insert(seg_names).first;

  return Engine::Get()->NewOperator(
      exec_fun, use_vars, mutate_vars, FnProperty::kNormal, it->c_str());
}
  ...
  for (size_t nid = start_nid; nid < end_nid; ++nid) {
    ...
    if (stop && nid > seg_start) {
      ...
      if (seg_execs.size()) {
        ...
        seg_names.pop_back();
        seg_names += "]";
        seg.opr.reset(CreateEngineOp(default_ctx, seg_execs, seg_names, opr_segs_names));
      } else {

...
void ConvertSlice(NodeProto* node_proto, const NodeAttrs& attrs,
                  const nnvm::IndexedGraph& /*ig*/,
                  const array_view<IndexedGraph::NodeEntry>& /*inputs*/) {
...
void ConvertConcatenate(NodeProto* node_proto, const NodeAttrs& attrs,
  ...
  axis->set_name("axis");
  axis->set_type(AttributeProto::INT);
  axis->set_i(static_cast<int64>(_param.dim));
}
...
std::unordered_map<std::string, mxnet::TShape> GetPlaceholderShapes(
    const mxnet::ShapeVector& shape_inputs, const nnvm::IndexedGraph& ig) {
...
void ConvertOutput(
    ...
    const std::string& node_name, const mxnet::ShapeVector& shapes,

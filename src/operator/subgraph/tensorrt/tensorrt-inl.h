...
class TensorrtSelector : public SubgraphSelector {
 public:
  ...
  bool isTRTCompatible(const nnvm::Node &n) {
    ...
    // TODO(cfujitsang): Check about limit on stride / pad / kernel
    if (op_name == "Pooling") {
    ...
    // TODO(cfujitsang): pretty sure there are some limit on stride / pad / kernel
    ...
    if (op_name == "Activation") {
      const auto& param = nnvm::get<ActivationParam>(n.attrs.parsed);
    ...
    if (op_name == "slice") {
      const auto& param = nnvm::get<SliceParam>(n.attrs.parsed);
      if (param.begin[0].has_value() && param.begin[0].value() != 0)
        return false;
      if (param.end[0].has_value())
        return false;
      for (auto a : param.step) {
        if (a.has_value() && a.value() != 1)
          return false;
      }
      return true;
    }
...
class TensorrtProperty : public SubgraphProperty {
 public:
  static SubgraphPropertyPtr Create() {
    auto sgprop = std::make_shared<TensorrtProperty>();
    sgprop->SetAttr<std::string>("property_name", "TensorRT");
    return sgprop;
  }

...
OpStatePtr TRTCreateState(const nnvm::NodeAttrs& attrs, Context ctx,
  ...
  std::vector<mxnet::TShape> out_shape(graph.outputs.size());
  std::vector<int> out_type(graph.outputs.size(), -1);
  std::vector<mxnet::TShape> _in_shape(in_shape.begin(), in_shape.end());
  std::vector<int> _in_type(in_type.begin(), in_type.end());

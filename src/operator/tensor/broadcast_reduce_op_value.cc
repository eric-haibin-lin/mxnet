...
.set_attr<nnvm::FGradient>("FGradient", ElemwiseGradUseNone{"_backward_broadcast_like"})
...
NNVM_REGISTER_OP(_backward_broadcast_like)
.set_num_outputs(2)
.set_attr_parser(ParamParser<BroadcastLikeParam>)
.set_attr<nnvm::TIsBackward>("TIsBackward", true)
.set_attr<FResourceRequest>("FResourceRequest",
...
.set_attr<FCompute>("FCompute<cpu>", BroadcastLikeGradCompute<cpu>);

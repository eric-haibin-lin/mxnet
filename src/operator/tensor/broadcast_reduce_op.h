...
template<typename xpu>
void BroadcastLikeGradCompute(const nnvm::NodeAttrs& attrs,
                              const OpContext& ctx,
                              const std::vector<TBlob>& inputs,
                              const std::vector<OpReqType>& req,
                              const std::vector<TBlob>& outputs) {
  // output[0] is the gradient for the broadcast input, so the gradient is a sum reduction.
  // Reduction is performed assuming keepdims=true.
  mxnet::TShape small = outputs[0].shape_;
  ReduceAxesComputeImpl<xpu, mshadow::red::sum, false>(ctx, inputs, {req[0]}, {outputs[0]}, small);
  // output[1] is the gradient for the input that provided the shape, so we just zero it.
  FillCompute<xpu, 0>(attrs, ctx, inputs, {req[1]}, {outputs[1]});
}

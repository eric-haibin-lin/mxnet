...
void GraphExecutor::Init(nnvm::Symbol symbol, const Context &default_ctx,
  ...
  std::transform(aux_states.begin(), aux_states.end(), aux_state_ctxes.begin(),
                 get_ctx1);
  auto infer_attr = [](nnvm::Graph&& _g) {
    LOG(INFO) << "Warning: Custom fusion are disabled with regular bind function";
    return _g;
  };
  nnvm::Graph g = InitGraph(symbol, grad_req_types, default_ctx, false, infer_attr);
...
void GraphExecutor::Init(nnvm::Symbol symbol,
                         ...
                         Executor* shared_exec,
                         const nnvm::NodeEntryMap<NDArray>& feed_dict) {
  auto infer_attr = [this, arg_shape_map, arg_dtype_map, arg_stype_map,
                     default_ctx, ctx_map, in_arg_ctxes, arg_grad_ctxes,
                     aux_state_ctxes, grad_req_types](nnvm::Graph&& _g) {
    nnvm::Graph g;
    g.outputs = _g.outputs;
    ...
    g = InferShape(std::move(g), std::move(arg_shapes), "__shape__");
    auto shapes = g.GetAttr<mxnet::ShapeVector>("shape");
    if (g.GetAttr<size_t>("shape_num_unknown_nodes") != 0U) {
  ...
  nnvm::Graph g = InitGraph(symbol, grad_req_types, default_ctx, true, infer_attr);
  g = infer_attr(std::move(g));
...
/*!
 * \brief Create the graph for forward pass.
 * this is triggered by both simple_bind and bind flows.
 */
nnvm::Graph
GraphExecutor::InitForwardGraph(nnvm::Symbol symbol) {
  nnvm::Graph g;
  num_forward_outputs_ = symbol.outputs.size();
  num_forward_inputs_ = symbol.ListInputs(nnvm::Symbol::kAll).size();
  g.outputs = symbol.outputs;
 ...
 */
nnvm::Graph
GraphExecutor::InitBackwardGraph(nnvm::Graph&& g,
                                 nnvm::Symbol symbol,
                                 const std::vector<OpReqType> &grad_req_types) {
  using nnvm::NodeEntry;
  using nnvm::NodePtr;
  ...
  for (size_t i = 0; i < grad_req_types.size(); ++i) {
    if (grad_req_types[i] != kNullOp) {
      xs.emplace_back(NodeEntry{args[i]});
    }
  }
...
Graph GraphExecutor::InitGraph(nnvm::Symbol symbol,
                               const std::vector<OpReqType> &grad_req_types,
                               const Context& default_ctx,
                               const bool simple_bind,
                               F infer_attr) {
  nnvm::Graph g = InitForwardGraph(symbol);
  bool use_fusion = dmlc::GetEnv("MXNET_USE_FUSION", true);
  if (use_fusion && default_ctx.dev_mask() == Context::kGPU && simple_bind) {
    // Here goes any fusion on the forward graph before backward graph generation
    // if for instance the fused node have a backward defined
    if (!dmlc::GetEnv("MXNET_DISABLE_BNACTIV_FUSION", false)) {
      g = infer_attr(std::move(g));
      g = FuseBNActiv(std::move(g));
    }
    if (!dmlc::GetEnv("MXNET_DISABLE_BNADDRELU_FUSION", false)) {
      g = infer_attr(std::move(g));
      g = FuseBNAddRelu(std::move(g));
    }
    if (!dmlc::GetEnv("MXNET_DISABLE_NORMCONV_FUSION", true)) {
      g = infer_attr(std::move(g));
      g = FuseConvBN(std::move(g));
    }
  }
  g = InitBackwardGraph(std::move(g), symbol, grad_req_types);
#if MXNET_USE_CUDA && !defined(_WIN32)
  if (use_fusion && default_ctx.dev_mask() == Context::kGPU) {
...
GraphExecutor::CachedSegOpr GraphExecutor::CreateCachedSegOpr(size_t topo_start,
  ...
  bool use_cuda_graph = dmlc::GetEnv("MXNET_EXPERIMENTAL_ENABLE_CUDA_GRAPH", false);
  auto exec_fun = [use_cuda_graph, have_cuda_graph = false, graph_exec = cudaGraphExec_t(),
                   exec_list, is_gpu](RunContext ctx,
    ...
    if (is_gpu) {
      ...
      if (use_cuda_graph) {
        cudaStream_t cu_s = mshadow::Stream<gpu>::GetStream(s);
        if (!have_cuda_graph) {
          LOG(INFO) << "Create CUDA graph";
          // Warmup for any algorithm find approach
          ...
          char pLogBuffer[100];
          cudaStreamBeginCapture(cu_s, cudaStreamCaptureModeRelaxed);
          // Run all opr in the sub-graph
          for (auto &exec : exec_list) {
            exec->Run(ctx, is_gpu);
          }
          cudaGraph_t cuda_graph;
          cudaStreamEndCapture(cu_s, &cuda_graph);
          cudaGetLastError();
          cudaError_t err = cudaGraphInstantiate(&graph_exec, cuda_graph,
                                                 &pErrorNode, pLogBuffer, 100);
          CHECK_EQ(err, cudaSuccess) << " " << cudaGetErrorString(err);
          have_cuda_graph = true;
        } else {
          cudaError_t err = cudaGraphLaunch(graph_exec, cu_s);
          CHECK_EQ(err, cudaSuccess) << " " << cudaGetErrorString(err);
        }
      } else {
        for (auto &exec : exec_list) {

  ...
  if (global_state_->batch_d2d_memcopies) {
    ...
    for (auto& e : entries) {
      ...
      if (idx % BATCHED_D2D_CAPACITY == 0 || idx == (int) entries.size()) {
        ...
        batched_d2d_memcpy(d2d_params, count, cuda_context_->streams[global_state_->current_nccl_stream][first_entry.device]);
        count = 0;
      }
    }
...
Status CUDAAllreduce::FinalizeCUDAQueue(const std::vector<TensorTableEntry>& entries) {
  ...
  cuda_context_->finalizer_thread_pool.execute([entries, first_entry, host_buffer, fusion_buffer,

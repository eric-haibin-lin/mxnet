...
from horovod.mxnet.mpi_ops import register_group
...
class DistributedOptimizer(mx.optimizer.Optimizer):
    def __init__(self, optimizer, skip_average=False, num_groups=None):
    ...
    def _do_allreduce(self, index, grad):
        if isinstance(index, (tuple, list)):
            if (self._num_groups and self._num_groups > 0):
                num_grads_per_group = (len(grad) + self._num_groups - 1) // self._num_groups
                group_ids = [register_group(num_grads_per_group,
                                            "%s:%s:%d" % (index[0], index[-1], i))
                             for i in range(len(grad) // num_grads_per_group)]
                if len(grad) % num_grads_per_group != 0:
                    group_ids.append(register_group(len(grad) % num_grads_per_group,
                                                    "%s:%s:%d" % (index[0], index[-1],
                                                    len(grad) // num_grads_per_group + 1)))

                for i in range(len(index)):
                    allreduce_(grad[i], average=False,
                               name=str(index[i]), priority=-i,
                               group_id=group_ids[i // num_grads_per_group])
            else:
                for i in range(len(index)):
                    allreduce_(grad[i], average=False,
                               name=str(index[i]), priority=-i)

...
def get_nvcc_flags(build_ext):
    last_err = None
    default_flags = ['-O2', '-Xcompiler', '-fPIC']
    cc_list = [52, 60, 61, 70, 75]
    for cc in cc_list:
      default_flags += ['-gencode', 'arch=compute_{cc},code=sm_{cc}'.format(cc=cc)]

    return default_flags

def test_nvcc(build_ext):
     try:
         test_compile_nvcc(build_ext, 'test_nvcc_flags',
                      code=textwrap.dedent('''\
             __global__
             void test_kernel(int* A) {
                 A[blockDim.x * blockIdx.x + threadIdx.x] = 1;
             }
             '''))

         return
     except (CompileError, LinkError):
         last_err = 'Unable to determine nvcc compilation flags (see error above).'
     except Exception:
         last_err = 'Unable to determine nvcc compilation flags.  ' \
                    'Last error:\n\n%s' % traceback.format_exc()
...
# Modified from: https://stackoverflow.com/questions/10034325/can-python-distutils-compile-cuda-code
def add_nvcc_support(compiler, build_ext):
    ...
    def _compile(obj, src, ext, cc_args, extra_postargs, pp_opts):
        ...
        if ext == '.cu':
            nvcc_flags = get_nvcc_flags(build_ext)
            _, _, cuda_home = get_cuda_dirs(build_ext, nvcc_flags, skip_test=True)
            compiler.set_executable('compiler_so', os.path.join(cuda_home, *['bin', 'nvcc']))
            postargs = nvcc_flags
        else:
...
def test_compile_nvcc(build_ext, name, code, libraries=None, include_dirs=None, library_dirs=None,
                 macros=None, extra_compile_preargs=None, extra_link_preargs=None):
    test_compile_dir = os.path.join(build_ext.build_temp, 'test_compile_nvcc')
    if not os.path.exists(test_compile_dir):
        os.makedirs(test_compile_dir)

    source_file = os.path.join(test_compile_dir, '%s.cu' % name)
    with open(source_file, 'w') as f:
        f.write(code)

    compiler = build_ext.compiler

    add_nvcc_support(compiler, build_ext)
...
def get_cuda_dirs(build_ext, cpp_flags, skip_test=False):
    cuda_include_dirs = []
    cuda_lib_dirs = []
    ...
    if skip_test:
        ...
        try:
            ...
            raise DistutilsPlatformError(
                ...
                'HOROVOD_CUDA_HOME - path where CUDA include and lib and bin directories can be found\n'
        ...
        try:
            ...
            raise DistutilsPlatformError(
                'nvcc was not found (see error above).\n'
                'Please specify correct CUDA location with the HOROVOD_CUDA_HOME. \n\n '
                'HOROVOD_CUDA_HOME - path where CUDA include and lib and bin directories can be found')
...
def get_common_options(build_ext):
    ...
    if gpu_allreduce or gpu_allgather or gpu_broadcast:
        have_cuda = True
        cuda_include_dirs, cuda_lib_dirs, cuda_home = get_cuda_dirs(build_ext, cpp_flags)
        nvcc_flags = get_nvcc_flags(build_ext)
    else:
        have_cuda = False
    ...
    if have_cuda:
        ...
        SOURCES += ['horovod/common/ops/cuda_operations.cc',
                    'horovod/common/batched_memcpy.cu']
        if have_mpi:
            SOURCES += ['horovod/common/ops/mpi_cuda_operations.cc']
        LIBRARY_DIRS += cuda_lib_dirs
        LIBRARIES += ['cudart']
        NVCC_COMPILE_FLAGS += nvcc_flags
...
class custom_build_ext(build_ext):
    def build_extensions(self):
        ...
        add_nvcc_support(self.compiler, build_ext)
        built_plugins = []

...
/*!
 * \brief Initialize IPC exchange buffers (multiple GPUs per process)
 * \param gpus number of gpus used
 * \param sync_steps number of group syncronization steps. Effectively = log2(group_size)
 * \param xbuf_ptr a preallocated array of void_ptr is filled in with buffer pointers
 */
MXNET_DLL int MXInitXBuf(int gpus, int sync_steps, void** xbuf_ptr);

/*!
 * \brief Initialize IPC exchange buffers on a single GPU
 * \param gpu_id - id of current gpu
 * \param sync_steps number of group syncronization steps. Effectively = log2(group_size)
 * \param xbuf_ptr a preallocated array of void_ptr is filled in with buffer pointers
 * \param hndl_ptr a pointer to preallocated cudaIpcMemHandle_t buffer for return
 */
MXNET_DLL int MXInitXBufSingle(int gpu_id, int sync_steps, void** xbuf_ptr, void* hndl_ptr);

/*!
 * \brief Initialize IPC exchange buffers from IPC handles list
 * \param gpu_id - id of current gpu
 * \param gpus number of gpus used
 * \param sync_steps number of group syncronization steps. Effectively = log2(group_size)
 * \param xbuf_ptr a preallocated array of void_ptr is filled in with buffer pointers
 * \param hndl_ptr a array of cudaIpcMemHandle_t buffers from cooperating GPUs
 */
MXNET_DLL int MXOpenIpcHandles(int gpu_id, int gpus, int sync_steps,
                               void** xbuf_ptr, void** hndl_ptr);

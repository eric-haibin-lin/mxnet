...
class BidirectionalCell(HybridRecurrentCell):
    ...
    def hybrid_unroll(self, F, inputs, begin_state=None, layout='NTC', merge_outputs=None,
               valid_length=None):
        # TODO remove length from API
        # pylint: disable=too-many-locals
        self.reset()
        #import pdb; pdb.set_trace()
        assert merge_outputs, "Only merge_outputs = True is supported"
        batch_size = 1
        #inputs, axis, F, batch_size = _format_sequence(0, inputs, layout, False)
        if valid_length is None:
            #reversed_inputs = list(reversed(inputs))
        ...
        if valid_length is None:
            ...
            #reversed_r_outputs = list(reversed(r_outputs))
            #reversed_r_outputs = F.SequenceReverse(r_outputs.reshape(-1, batch_size, 512),
            #                                       axis=0)
            reversed_r_outputs = F.SequenceReverse(F.swapaxes(r_outputs,0,1),
                                                   axis=0)
        else:
            assert False
        ...
        if merge_outputs:
            #reversed_r_outputs = F.stack(*reversed_r_outputs, axis=axis)
            #reversed_r_outputs = reversed_r_outputs.reshape(batch_size, -1, 512)

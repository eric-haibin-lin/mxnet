...
class CachedOpExecutor:
    """Executor with cached op as the implementation"""
    def __init__(self, sym, ctx, args, args_grad, grad_req):
        ...
        if not self._args_grad:
            ...
            for arg, out in zip(self._args, self._args_grad):
                arg.grad[:] = out
        self._cached_op = mx.nd.CachedOp(sym)
...
def bind(symbol, ctx, args, args_grad=None, grad_req='write',
    ...
    return CachedOpExecutor(symbol, ctx, args, args_grad, grad_req)

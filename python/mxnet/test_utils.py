...
def check_consistency(sym, ctx_list, scale=1.0, grad_req='write',
    ...
    scale : float, optional
        ...
        Also scale factor applied to uniform distribution, otherwise [-0.92, +0.92]
 ...
 {'ctx': mx.cpu(0), 'concat_arg1': (2, 10), 'concat_arg0': (2, 10),\
  'type_dict': {'concat_arg0': np.float32, 'concat_arg1': np.float32}}]
    ...
    if grad_req != 'null':
        ...
        # made that the out_grad data for backward().  This is a little weird, but easily
...
@contextmanager
def discard_output(file):
    """
    Discards output of a routine to a file such as sys.stdout or sys.stderr:

    with _discard_output(file):
        ...
    """
    with open(os.devnull, 'w') as bit_bucket:
        try:
            fileno = file.fileno()
            # Flush any prior output to file that may be buffered
            file.flush()
            old_file = os.dup(fileno)
            try:
                os.dup2(bit_bucket.fileno(), fileno)
                yield
            finally:
                # Flush any buffered output (to bit-bucket) before reinstating old file
                file.flush()
                os.dup2(old_file, fileno)
        except (io.UnsupportedOperation, AttributeError):
            # On some systems, file is not a descriptor but actually a virtual pipeline
...
def discard_stderr():
    ...
    with discard_stderr():
        ...
    """
    with discard_output(sys.stderr) as X:
        yield X

@contextmanager
def discard_stdout():
    """
    Discards standard output of a routine if invoked as:

    with discard_stdout():
        ...
    """
    with discard_output(sys.stdout) as X:
        yield X
